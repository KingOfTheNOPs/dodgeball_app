...
export default function EliminationTracker({ losingTeamPlayers, onComplete, onCancel }) {
  ...
  const handleComplete = () => {
    onComplete(eliminatedOrder);
  };

  const remainingPlayers = losingTeamPlayers.filter(
    p => !eliminatedOrder.find(e => e.id === p.id)
  );

  return (
    <Card className="border-0 shadow-2xl bg-gradient-to-br from-rose-50 to-red-50">
      <CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="min-h-[60px] bg-white rounded-lg p-3 border-2 border-rose-200">
            {eliminatedOrder.length === 0 ? (
              <p className="text-center text-slate-400 text-sm py-2">Click players below to track order</p>
            ) : (
              <div className="flex flex-wrap gap-2">
                {eliminatedOrder.map((player, index) => (
                  <motion.div
                    key={player.id}
            ...
            <AnimatePresence>
              {remainingPlayers.map((player) => (
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  className="flex items-center gap-2 p-3 bg-white rounded-lg border-2 border-slate-200 hover:border-rose-400 transition-all"
                >
        ...
        <div className="flex gap-2 pt-4">
          ...
          <Button
            ...
            disabled={eliminatedOrder.length !== losingTeamPlayers.length}
